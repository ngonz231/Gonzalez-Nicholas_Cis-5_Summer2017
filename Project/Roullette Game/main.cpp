 ...
 * Purpose:  Play A game of roulette
 */
...
int main(int argc, char** argv) {
    ...
    char name[10];//Allows Up to 10 character for players Name
    
    int betType,r,win,b,ev,od,high,low,c1,c2,c3,acct,bet,num,rNum;
    string color,hLo;
    char choice,evO,col;
    ...
    cin>>bet;
    while(bet<0||bet>100||bet>acct){//Allows only certain values
        cout<<"Please enter a number between 1 and 100 or within "<<
                acct<<"(Account Value)"<<endl;
        cin>>bet;
       }
        
    do{//Do while loop to allow continuous play
        cout<<"Please enter the number of the bet you would like to make."<<endl;
    ...
    switch(betType){//Switch statement to allow multiple game types
        ...
        case 1: 
            ...
            if (color=="Red"||color=="red"){
                ...
                r=rand()%36+1;//Random number generator
                if(r%2==0){
                ...
                b=rand()%36+1;
                if(b%2==1){//Random number selector
                    cout<<"Congratulations  "<<name<<" You Won "<<bet<<endl;
                    acct+=bet;
                    cout<<"You now have "<<acct+win<<" In your account"<<endl;
                }else{
                    cout<<"Sorry "<<name<<" Better luck next time"<<endl;
                    acct-=bet;//Keeps running total of account balance
                cout<<"You now have "<<acct<<" in your account"<<endl;
                }
            }cout<<"Would You Like to Bet on Red or Black Again?(Y or N)"<<endl;
            cin>>choice;//Allows to run specific game over again
        ...
        case 2:
            ...
            while(evO!='E'&&evO!='e'&&evO!='O'&&evO!='o'){
                cout<<"Invalid Choice. Please enter an E or O"<<endl;//Allows for only certain types of inputs
                cin>>evO;
            }
            if(evO=='E'||evO=='e'){
             ev=rand()%36+1;//random
        ...
        case 5:
            do{
            cout<<"Specific Number Bet, Good Luck."<<endl;
            cout<<"Please select any number from 1-36"<<endl;
            cin>>num;
            rNum=rand()%36+1;
            cout<<"Number Rolled: "<<rNum<<endl;
            if (num==rNum){
                cout<<"Congratulations!!!!!  "<<name<<" You Won "<<bet*35<<endl;
                    acct+=bet*35;
                    cout<<"You now have "<<acct+win<<" In your account!!!"<<endl;
                }else{
                    cout<<"Sorry "<<name<<" Better luck next time"<<endl;
            ...
            cout<<"Would you like to try this type of bet again?(Y/N)"<<endl;
            cin>>choice;
            }while(choice=='Y'||choice=='y');
            break;
...
//Process/Calculations Here
    ...
    //Exit
    }while(betType>=1&&betType<=5);//While loop to allow constant selection of games
    return 0;
}
